...
def other_section(root, outdir):
    cards = "".join(
        f'<div><h3>{escape(label)}</h3>{video_cell(path, root, outdir, cls="other-video")}</div>'
        for label, path in OTHER_VIDEOS
    )
    return f'<section id="extraPanel" class="wide"><h2>Other videos</h2><p><em>ellipsoid · SH3 · sort · ESS · interleaved prune</em></p><div class="extra-grid">{cards}</div></section>'


def page(root, out, metrics):
    outdir = out.parent
    return f'''<!doctype html><html><head><meta charset="utf-8"><title>Ablation Videos</title><style>{CSS}</style></head><body class="layout-both"><h1>Ablation Videos</h1><div class="controls"><button onclick="pauseAll=false;updateVideos()">Resume visible</button><button onclick="pauseAll=true;document.querySelectorAll('video').forEach(v=>v.pause())">Pause all</button><button id="layoutToggle" onclick="cycleLayout()">View: Both</button><button id="themeToggle" onclick="cycleTheme()">Theme: System</button></div><div id="ablationsPanel" class="grid">{''.join(ablation_block(v, root, outdir, metrics) for v in VIDEOS)}</div>{mog_section(root, outdir)}{other_section(root, outdir)}<script>{JS}</script></body></html>'''
