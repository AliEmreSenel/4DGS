...
fn cov3_from_scale_rot(scale: vec3<f32>, scale_modifier: f32, rot: vec4<f32>) -> mat3x3<f32> {
  let sc = scale_modifier * scale;
  let q = qnorm(rot);
  let r = q.x; let x = q.y; let y = q.z; let z = q.w;
  // Exact GLM column-major constructor from forward.cu::computeCov3D.
  let R = mat3x3<f32>(
...
fn mat_l_cuda(rot: vec4<f32>) -> mat4x4<f32> {
  ...
  // Exact GLM column-major constructor from forward.cu::computeCov3D_conditional.
  return mat4x4<f32>(
    vec4<f32>( a,  b, -c,  d),
    vec4<f32>(-b,  a,  d,  c),
    vec4<f32>( c, -d,  a,  b),
...
fn cov2d_cuda(mean: vec3<f32>, cov3: mat3x3<f32>, frame: FrameUniforms) -> vec3<f32> {
  // Exact forward.cu::computeCov2D with CUDA/GLM column-major semantics.
  ...
  let J = mat3x3<f32>(
    vec3<f32>(focal_x / z, 0.0, -(focal_x * t.x) / (z * z)),
    vec3<f32>(0.0, focal_y / z, -(focal_y * t.y) / (z * z)),
    vec3<f32>(0.0, 0.0, 0.0)
  );

  let V = frame.view;
  let Wm = mat3x3<f32>(
    vec3<f32>(V[0][0], V[1][0], V[2][0]),
    vec3<f32>(V[0][1], V[1][1], V[2][1]),
    vec3<f32>(V[0][2], V[1][2], V[2][2])
  );
