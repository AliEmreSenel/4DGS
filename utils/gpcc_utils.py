...
def gpcc_encode(encoder_path: str, ply_path: str, bin_path: str) -> None:
    cmd = [
        ...
        f"--uncompressedDataPath={os.path.abspath(ply_path)}",
        f"--compressedStreamPath={os.path.abspath(bin_path)}",
    ]
    try:
        subprocess.run(cmd, check=True, capture_output=True, text=True)
    except FileNotFoundError as e:
        raise RuntimeError(f"GPCC encoder not found: {encoder_path}") from e
    except subprocess.CalledProcessError as e:
        raise RuntimeError(
            f"GPCC encoder failed with exit code {e.returncode}\n"
            f"STDOUT:\n{e.stdout}\nSTDERR:\n{e.stderr}"
        ) from e

def gpcc_decode(decoder_path: str, bin_path: str, recon_path: str) -> None:
    cmd = [
        os.path.abspath(decoder_path),
        "--mode=1",
        "--outputBinaryPly=1",
        f"--compressedStreamPath={os.path.abspath(bin_path)}",
        f"--reconstructedDataPath={os.path.abspath(recon_path)}",
    ]
    try:
        subprocess.run(cmd, check=True, capture_output=True, text=True)
    except FileNotFoundError as e:
        raise RuntimeError(f"GPCC decoder not found: {decoder_path}") from e
    except subprocess.CalledProcessError as e:
        raise RuntimeError(
            f"GPCC decoder failed with exit code {e.returncode}\n"
            f"STDOUT:\n{e.stdout}\nSTDERR:\n{e.stderr}"
        ) from e
