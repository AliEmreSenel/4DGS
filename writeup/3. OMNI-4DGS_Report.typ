...
Visual quality improves most from anisotropic Gaussians and SH(3) color, with interleaved pruning and densification giving the strongest pruning schedule. Anisotropic Gaussians outperform isotropic ones with higher PSNR ($31.78$ vs. $29.66$, $+7.2%$, $g=1.42$), higher SSIM ($0.9797$ vs. $0.9713$, $+0.9%$, $g=1.63$), and lower LPIPS ($0.0218$ vs. $0.0416$, $-47.6%$, $g=2.55$), showing that directional covariance is important for representing geometry and motion-dependent appearance. SH(3) also strongly improves quality over RGB, raising PSNR from $29.81$ to $31.63$, raising SSIM from $0.9729$ to $0.9780$, and reducing LPIPS from $0.0349$ to $0.0285$. Interleaved pruning and densification gives the best mean PSNR, SSIM, and LPIPS among pruning schedules, while early initialization pruning is not competitive with the strongest quality-compact configurations. Pruning is therefore most useful when paired with densification and evaluated jointly with the Gaussian representation.
...
Training time is most affected by dropout, pruning, and color representation. Removing dropout gives the largest improvement, reducing training time from $5362.1$ to $2128.7$ seconds ($-60.3%$, $g=2.01$). Dropout therefore adds optimization cost without clear quality or compactness gains. Interleaved pruning and densification remains attractive for VRAM, serialized model size, and quality, but it increases training time relative to the no-pruning schedule because pruning and densification add control overhead. RGB trains faster than SH(3), but the quality loss makes it less suitable for high-fidelity reconstruction. Dropout is the most harmful choice for training efficiency because it substantially slows convergence.

== Role of each Component

Anisotropic Gaussians are one of the most beneficial components: they improve PSNR, SSIM, LPIPS, and reduce Gaussian count. Isotropic Gaussians use fewer variables per primitive but lose too much representational power. RGB reduces memory and checkpoint size but hurts quality, while SH(3) improves visual fidelity and reduces primitive count at the cost of higher VRAM and checkpoint size. Interleaved pruning best balances quality, count, memory, and VRAM; final pruning is strongest for FPS; early initialization pruning is not selected by the strongest quality-compact configurations. Dropout gives no clear benefit, and removing it improves time, FPS, count, and mean PSNR. Sort-free rendering, ESS, and USPLAT provided limited gains or excessive overhead in this setting.
