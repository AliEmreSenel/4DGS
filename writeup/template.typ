 ...
 * cvpr2025.typ
 *
 * Combined single-file CVPR 2025 template.
 */

#let std-bibliography = bibliography  // Due to argument shadowing.

#let conf-name = [CVPR]
#let default-conf-year = [2022]
...
#let xruler(side, dx, dy, width, height, offset, num-lines) = {
  let alignment = if side == left { right } else { left }
  ...
  place(left + top, dx: dx, dy: dy, ruler)
}
...
#let make-ruler(
  ...
  let margin = if margin == auto {
    (top: 1in - 0.5pt, left: 0.8125in, right: 0.929in)
  } else {
    margin
  }
...
#let format-affiliation(affl) = {
  // Department and institution on separate lines.
  let lines = ()
  ...
  // Address components on a single line.
  let address = ()
  if "location" in affl {
...
#let cvpr2022(
  ...
  // If there is no submission id then use a placeholder.
  if id == none {
    id = "*****"
  }
  ...
  set page(
    ...
    header: if accepted != none and not accepted {
      ...
      let year = aux.at("conf-year", default: default-conf-year)
  ...
  // Configure heading appearance and numbering.
  ...
  show ref: it => {
    ...
    if el.func() == math.equation {
      ...
      let ix = numbering("1.1", ..cnt.at(el.location()))
      let href = link(el.location(), ix)
      [#supplement~#href]
